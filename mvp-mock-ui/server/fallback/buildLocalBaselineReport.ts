...
function computeSimilarity(
  ...
  const keywordTokens = toTokens(extractedKeywords);
  const matched = unique(
    candidateTokens.filter((token) => queryTokens.includes(token) || keywordTokens.includes(token))
  ).slice(0, 6);

  const overlap = matched.length;
  const union = unique([...candidateTokens, ...queryTokens, ...keywordTokens]).length || 1;
  const score = clamp(overlap / union + 0.56, 0.32, 0.99);
...
function buildScoredComparables(
  candidates: DemoVideoRecord[],
  queryTokens: string[]
): ScoredComparable[] {
  const scored = candidates.map((candidate, index) => {
    const { score, matched } = computeSimilarity(
      candidate,
      queryTokens,
    ...
    const item: ComparableItem = {
      ...
      matched_keywords: matched.length > 0 ? matched : candidate.keywords.slice(0, 4),
      observations: [
        "The opening quickly states a concrete outcome.",
        "Pacing keeps attention through short visual beats.",
        "The call-to-action is explicit and easy to act on."
      ]
    };
...
function estimateUploadedVideoStats(
  ...
  const hook = clamp(50 + keywordBoost * 4 + (descWords > 20 ? 6 : 0), 40, 93);
  const clarity = clamp(56 + Math.min(descWords, 60) * 0.5 + mentions.length * 2, 45, 95);
  ...
  const engagementRate = clamp(3.5 + hook * 0.05 + clarity * 0.03, 2.5, 12.5);
...
function buildRelevantComments(
  ...
  for (const comparable of rankedComparables.slice(0, 8)) {
    for (const comment of comparable.candidate.comments.slice(0, 2)) {
      if (items.length >= 10) {
        return items;
      }
...
function buildRecommendations(): ReportOutput["recommendations"]["items"] {
  return [
    {
      id: "rec-1",
    ...
    {
      id: "rec-3",
      title: "End with a specific question CTA to increase meaningful comments.",
      priority: "High",
      effort: "Medium",
      evidence:
...
export function buildLocalBaselineReport(
  ...
  const { candidates, mentions, hashtags, description, candidatesK } = params;

  const queryTokens = toTokens([
    description,
    ...mentions,
    ...hashtags,
    ...HARD_CODED_EXTRACTED_KEYWORDS
  ...
  const rankedComparables = buildScoredComparables(candidates, queryTokens);
  const comparables = rankedComparables.slice(0, 8).map((entry) => entry.item);

  const yourEstimate = estimateUploadedVideoStats(description, hashtags, mentions);
  ...
  return {
    ...
    direct_comparison: {
      rows: directRows,
      note: "Estimated from local dataset candidates."
    },
    relevant_comments: {
      items: buildRelevantComments(rankedComparables),
